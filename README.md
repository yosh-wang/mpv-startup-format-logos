...
| Slug | 显示名称 | 检测方式 |
|------|----------|----------|
| `dolby-vision` | Dolby Vision | 轨道参数 + 文件名 |
| `hdr-vivid` | HDR Vivid | 轨道参数 + 文件名 |
| `hdr10-plus` | HDR10+ | 轨道参数 + 文件名 |
| `hdr10` | HDR10 | PQ 传输曲线 + 文件名 |
| `hlg` | HLG | HLG 传输曲线 + 文件名 |
| `sdr` | SDR | 常规 SDR 内容 |
...
| Slug | 显示名称 | 检测方式 |
|------|----------|----------|
| `dolby-atmos` | Dolby Atmos | 元数据 + 文件名 |
| `dts-x` | DTS:X | 文件名 |
| `audio-vivid` | Audio Vivid | 编解码器 + 文件名 |
| `dolby-truehd` | Dolby TrueHD | 编解码器 + 文件名 |
| `dts-hd-ma` | DTS-HD MA | 编解码器 + 文件名 |
| `dts-hd-hra` | DTS-HD HRA | 编解码器 + 文件名 |
| `dolby-digital-plus` | Dolby Digital Plus | 编解码器 + 文件名 |
| `dolby-digital` | Dolby Digital | 编解码器 + 文件名 |
| `dts` | DTS | 编解码器 + 文件名 |
| `ac4` | AC-4 | 编解码器 |
| `mpeg-h` | MPEG-H | 编解码器 |
| `flac` | FLAC | 编解码器 |
...
请前往 [Issues](https://github.com/yourname/mpv-startup-format-logos/issues) 提交问题，并附上：
...
特此授予任何获得本软件及相关文档文件的人免费使用本软件的权利，包括但不限于使用、复制、修改、合并、发布、分发、再许可和/或销售本软件副本的权利，并允许其接收者享有上述权利，但须符合以下条件：

上述版权声明和本许可声明应包含在本软件的所有副本或主要部分中。

本软件按"现状"提供，不作任何明示或暗示的保证，包括但不限于适销性、特定用途适用性和非侵权性的保证。在任何情况下，作者或版权持有人均不对因本软件或本软件的使用或其他交易而产生的任何索赔、损害或其他责任承担责任，无论是在合同诉讼、侵权诉讼还是其他诉讼中。
